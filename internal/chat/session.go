...
func (cs *ChatSession) addLocalMessage(msg string) {
    ...
    if len(cs.localBuffer) > 200 {
        cs.localBuffer = cs.localBuffer[len(cs.localBuffer)-200:]
    }
}
...
func (cs *ChatSession) LeaveChannel() {
    ...
    broadcastMessage(cs.CurrentChannel,
        fmt.Sprintf("*** %s has left ***", util.Colorize(cs.Username)), cs)
